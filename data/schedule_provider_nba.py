...
==============================
Today's NBA schedule from ESPN's public scoreboard endpoint -- free, no key,
same "never raises" contract as the other providers. Off-season returns [] so
the sport stays dormant until the season opens (late Oct).
"""
...
def get_todays_nba_games(date_str):
    ...
    try:
        ...
        logger.error("Failed to fetch NBA schedule for %s: %s", date_str, exc)
        return []
...
def _parse_event(event, date_str):
    ...
    return Game(
        ...
        date=date_str,
        home_team=normalize_nba_team(home.get("team", {}).get("displayName", "")),
        away_team=normalize_nba_team(away.get("team", {}).get("displayName", "")),
